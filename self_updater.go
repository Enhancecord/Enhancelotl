...
func GetInstallerDownloadLink() string {
	const BaseUrl = "https://github.com/Enhancecord/Enhancelotl/releases/latest/download/"
	switch runtime.GOOS {
	case "windows":
		filename := Ternary(buildinfo.UiType == buildinfo.UiTypeCli, "EnhancelotlCli.exe", "Enhancelotl.exe")
		return BaseUrl + filename
	case "darwin":
		return BaseUrl + "Enhancelotl.MacOS.zip"
	case "linux":
		return BaseUrl + "EnhancelotlCli-linux"
	default:
		return ""
	}
}
...
func UpdateSelf() error {
	...
	tmp, err := os.CreateTemp(ownExeDir, "EnhancelotlUpdate")
	if err != nil {
